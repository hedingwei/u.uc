 ...
 * @author 定巍
 */
@WebServlet(name = "AddNewDPIEndPoint", urlPatterns = {"/logic/dpi/AddNewDPIEndPoint"})
public class AddNewDPIEndPoint extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            long count = dao.queryBuilder().where().eq("ip", ip).or().eq("devName", devName).countOf();
            if(count>0){
                out.print("{\"code\":1}");
            }else{
                DPIEndPointBean bean = new DPIEndPointBean();
                bean.setDevName(devName);
                bean.setClientChannelState(ConnectionState.DisConnected);
                bean.setServerChannelState(ConnectionState.DisConnected);
