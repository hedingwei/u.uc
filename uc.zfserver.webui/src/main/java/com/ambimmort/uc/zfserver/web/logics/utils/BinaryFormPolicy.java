...
public class BinaryFormPolicy extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(msg.isVersionManaged()){
            try {
                msg.setMessageSerialNo(UcType.newUINT4(Long.parseLong(serialNo)));
                msg.getHeader().setMessageNo(UcType.newUINT2(Integer.parseInt(messageNo)));
            } catch (UcTypeException ex) {
                Logger.getLogger(BinaryFormPolicy.class.getName()).log(Level.SEVERE, null, ex);
            }
        }

            String hex = HexDisplay.getHex(msg.toBytes());
